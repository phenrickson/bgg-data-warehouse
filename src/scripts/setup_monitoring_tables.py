...
def create_monitoring_tables(client: bigquery.Client, config: dict) -> None:
    ...
    quality_check_table = bigquery.Table(
        f"{dataset_ref}.quality_check_results", schema=quality_check_schema
    )
    ...
    api_requests_table = bigquery.Table(f"{dataset_ref}.api_requests", schema=api_requests_schema)
