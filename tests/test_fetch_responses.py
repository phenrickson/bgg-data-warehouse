...
def test_fetch_responses_game_id_integrity():
    ...
    assert len(items) == len(chunk_ids), (
        f"Mismatch in number of items. Requested: {len(chunk_ids)}, Received: {len(items)}"
    )
    ...
    # Validate IDs match
    assert set(response_ids) == set(chunk_ids), (
        f"Mismatch in game IDs. Requested: {chunk_ids}, Received: {response_ids}"
    )
...
def test_store_response_game_id_integrity():
    ...
    """
    # Initialize fetcher
    fetcher = BGGResponseFetcher(batch_size=20, chunk_size=20, environment="dev")
    api_client = BGGAPIClient()
