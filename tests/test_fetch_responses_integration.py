...
def mock_bq_client():
    ...
    # Mock query result
    df_mock = pd.DataFrame({"game_id": [13, 9209, 325], "type": ["boardgame"] * 3})
...
def mock_api_client():
    ...
    def mock_get_thing(game_ids):
        ...
        return {
            "items": {
                "item": [{"@id": str(game_id), "name": responses[game_id]} for game_id in game_ids]
            }
        }
...
def test_fetch_responses_basic_flow(mock_bq_client, mock_api_client):
    ...
    fetcher = BGGResponseFetcher(batch_size=5, chunk_size=2, environment="dev")
